...
// 订阅目标点的回调函数，设置了trigger_和have_target_（注：manual模式的z被限定为1.0，end_vel_被设为0）
void KinoReplanFSM::waypointCallback(const nav_msgs::PathConstPtr& msg) {
  if (msg->poses[0].pose.position.z < -0.1) return;

  cout << "Triggered!" << endl;
  trigger_ = true; // trigger触发

  if (target_type_ == TARGET_TYPE::MANUAL_TARGET) {
    end_pt_ << msg->poses[0].pose.position.x, msg->poses[0].pose.position.y, msg->poses[0].pose.position.z; // z被设置为1.0了

  //预设的目标点，也就是xml文件里的方式flight_type=2？？
  ...
  end_vel_.setZero(); // 速度设为0
  have_target_ = true; // 目标点存在
  ...
  switch (exec_state_) {
    ...
    */
    case EXEC_TRAJ: {
      ...
      Eigen::Vector3d pos = info->position_traj_.evaluateDeBoorT(t_cur); // 当前位置

      /* && (end_pt_ - pos).norm() < 0.5 */
      if (t_cur > info->duration_ - 1e-2) { // 当前时间 > 执行轨迹的时长（超时了）
        have_target_ = false;
        changeFSMExecState(WAIT_TARGET, "FSM");
        return;

      } else if ((end_pt_ - pos).norm() < no_replan_thresh_) { // 到达终点
        // cout << "near end" << endl;
        return;

      } else if ((info->start_pos_ - pos).norm() < replan_thresh_) { // 当前距离与起点距离小于horizon
        // cout << "near start" << endl;
        return;

      } else {
        changeFSMExecState(REPLAN_TRAJ, "FSM"); // 进入重规划阶段
      }
      break;
    }
...
如果目标点有障碍物，就在目标点周围通过离散的半径及角度循环来寻找新的安全的目标点。通过设置dist来改变？0.3
...
// 执行kinodynamicReplan，并判断是否成功，成功则publish(bspline)
bool KinoReplanFSM::callKinodynamicReplan() {
  bool plan_success =
      planner_manager_->kinodynamicReplan(start_pt_, start_vel_, start_acc_, end_pt_, end_vel_); // 规划xyz

  if (plan_success) {

    planner_manager_->planYaw(start_yaw_); // xyz规划成功后再规划yaw
