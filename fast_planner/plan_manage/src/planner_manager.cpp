...
// 设置ROS nh句柄空间下的参数，初始化kino_path_finder和Bspline_optimizers
...
// distance即为阈值。根据local_data中属于NonUniformBspline类的position_traj的eveluateDeboor()函数计算局部轨迹的离散点的值，并根据SDF类的evaluateCorseEDT()计算障碍物距离，并返回发生碰撞时起始点到碰撞点的距离
...
// 设置起始点后，使用kino_path_finder的search()寻找路径，进行start_acc寻找，找不到再离散化寻找，然后再失败。然后均匀B样条拟合，再优化，当轨迹到达感知距离外，增加ENDPOINT优化项。最后非均匀样条曲线调整，赋值给local_data。position_traj。利用updateTrajInfo进行local_data其他数据更新
bool FastPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  // 调用search寻找路径（dynamic默认为false，time的初始化为-1）
  int status = kino_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, true); 

  // 判断有没找到路径，是否进行init=false的搜索
  if (status == KinodynamicAstar::NO_PATH) {
    cout << "[kino replan]: kinodynamic search fail!" << endl;

    // retry searching with discontinuous initial state
    kino_path_finder_->reset();
    status = kino_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, false); // 离散化搜索，init=false
  ...
  NonUniformBspline::parameterizeToBspline(ts, point_set, start_end_derivatives, ctrl_pts); // 进行B样条曲线拟合
  ...
  // 轨迹到达感知距离意外，增加ENDPOINT优化项
  if (status != KinodynamicAstar::REACH_END) {
    cost_function |= BsplineOptimizer::ENDPOINT;
  }

  ctrl_pts = bspline_optimizers_[0]->BsplineOptimizeTraj(ctrl_pts, ts, cost_function, 1, 1); // 均匀B样条曲线优化
  ...
  while (!feasible && ros::ok()) {
    ...
    feasible = pos.reallocateTime(); // 非均匀B样条迭代时间优化
  ...
  local_data_.position_traj_ = pos;  // B样条轨迹
  ...
  updateTrajInfo(); // 对local_data_的其他数据更新
