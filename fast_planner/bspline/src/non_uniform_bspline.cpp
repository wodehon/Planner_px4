...
// 均匀B的时间区间设置函数，在获得控制点，轨迹次数，以及时间间隔的情况下，设置时间区间（Knot vector),
...
// B样条函数值计算，根据给定时间u计算，使用递归的deboor算法计算。内循环通过递归公式求得高一阶的P+1个控制点，外循环提高结束
...
// 非均匀B样条曲线，新的控制点，次数，Knot vector赋值
...
// 重新进行时间调整的函数，通过计算当前控制点是否超限，以及调整表比例，对当前控制点有关的时间区间([t_i,t_(i+p_b+1)])进行时间调整。注，p_b为当前B样条曲线的次数，分别对于3,2,1
...
// 获得控制点-前端路径拟合：通过对前端hybrid A*寻找到的初始路径进行拟合得到的，论文说的是Deboor公式，但实现使用的是B样条的矩阵表达法
